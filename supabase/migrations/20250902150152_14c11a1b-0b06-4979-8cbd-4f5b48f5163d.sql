-- Fix database functions by properly dropping triggers first, then functions, then recreating

-- Drop trigger first
DROP TRIGGER IF EXISTS validate_booking_trigger ON public.bookings;

-- Drop functions with CASCADE to handle dependencies
DROP FUNCTION IF EXISTS public.validate_booking() CASCADE;
DROP FUNCTION IF EXISTS public.is_valid_email(text) CASCADE;
DROP FUNCTION IF EXISTS public.is_valid_phone(text) CASCADE;
DROP FUNCTION IF EXISTS public.create_secure_booking(jsonb, inet) CASCADE;

-- Recreate is_valid_email function without problematic SET statement
...
-- Recreate is_valid_phone function without problematic SET statement
...
-- Recreate validate_booking function without problematic SET statement
...
-- Recreate create_secure_booking function without problematic SET statement
